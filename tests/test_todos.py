...
@pytest.mark.asyncio
async def test_list_todos_pagination_should_return_todos(
...
async def test_list_todos_filter_description_should_return_5_todos(
    ...
    session.add_all(
        TodoFactory.create_batch(
            expected_todos, user_id=user.id, description='description 1'
        )
    )
    await session.commit()
...
@pytest.mark.asyncio
async def test_get_todo(session, client, user, token):
...
@pytest.mark.asyncio
async def test_get_todo_not_found(session, client, user, token):
    response = client.get(
        '/todos/1', headers={'Authorization': f'Bearer {token}'}
    )
